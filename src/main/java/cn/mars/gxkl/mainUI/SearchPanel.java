...
public class SearchPanel extends JPanel {
	...
	private void initialization() {
		...
		input.setFont(new Font("雅黑",Font.PLAIN,(int)(width*0.01)));
		input.setBackground(Color.white);
		input.setBorder(new LineBorder(Color.black));
		input.setFont(new Font("雅黑",Font.PLAIN,(int)(width*0.015)));
		this.add(input);
		
		JLabel button = new JLabel("搜索",JLabel.CENTER);
		button.setPreferredSize(new Dimension((int)(width*0.1-18),height/2-5));
		button.setBackground(new Color(0x92,0,1));
		button.setForeground(Color.white);
		button.setOpaque(true);
		button.setFont(new Font("雅黑",Font.BOLD,(int)(width*0.018)));
		this.add(button);
		
		String[] name = {
				"无查询","键盘输入查询","RFID扫描查询","其他扫描查询"
		};
		...
		for(int i=0;i<type_items.length;i++) {
			...
			type_items[i].setFont(new Font("雅黑",Font.PLAIN,(int)(width*0.015)));
